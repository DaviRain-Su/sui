...
pub fn module(
    ...
    for fun in arena::mut_to_ref_slice(loaded_functions) {
        cache.functions.push(ArenaPointer::new(fun as *const Function));
    }
    ...
    Ok(LoadedModule {
        ...
        function_map: function_map.into_iter().map(|(key, value)| (key, arena::ArenaPointer::new(value))).collect::<HashMap<_, _>>(),
        single_signature_token_map,
