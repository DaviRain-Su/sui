...
mod vm_tracer_noop {
    ...
    impl<'a> VMTracer<'a> {
        #[allow(dead_code)]
        pub(crate) fn new(_trace: &'a mut MoveTraceBuilder) -> Self {
            Self {
...
mod vm_tracer_impl {
    ...
    use move_trace_format::trace_format::{
        DataLoad, Effect as IF, Location, MoveTraceBuilder, Read, RefType, TraceIndex, TraceValue,
    ...
    #[derive(Debug, Clone)]
    struct AnnotatedTypeInstance<L, T> {
        layout: L,
        ref_type: T,
    }
    ...
    type RootedType = AnnotatedTypeInstance<MoveTypeLayout, Option<(RefType, RuntimeLocation)>>;
    ...
    /// value at the time).
    type LocalType = AnnotatedTypeInstance<Option<MoveTypeLayout>, ReferenceType>;
    ...
    impl<'a> VMTracer<'a> {
        ...
        fn unwrap_or_emit_err(&mut self, x: Option<()>) {
            x.unwrap_or_else(|| {
                self.trace.effect(IF::ExecutionError(
                    "Tracing error! Events from here below should be viewed with sceptically"
                        .to_string(),
                ));
            });
        }
        ...
        fn current_frame_locals(&self) -> Option<&[LocalType]> {
            Some(self.active_frames.last_key_value()?.1.locals.as_slice())
        }
        ...
        fn invalidate_local(&mut self, local_index: usize) -> Option<()> {
            let mut local = self.active_frames.last_entry()?;
            let local = local.get_mut().locals.get_mut(local_index)?;
        ...
        fn open_main_frame_(
            ...
            let locals_types = function_type_info
                ...
                .map(|tag_with_layout_info_opt| {
                    let (layout, ref_type) = match tag_with_layout_info_opt.layout {
                        None => (None, None),
                        Some((layout, ref_type)) => (Some(layout), ref_type),
                    };
                    LocalType {
                        layout,
                        ref_type: ref_type
            ...
            self.trace.open_frame(
                ...
                function_type_info
                    .return_types
                    .iter()
                    .map(|tag_with_layout_info_opt| tag_with_layout_info_opt.into_tag_with_refs())
                    .collect(),
                function_type_info
                    .local_types
                    .into_iter()
            ...
            let locals_types = function_type_info
                ...
                .map(|(i, tag_with_layout_info_opt)| {
                    ...
                    if let Some(a_layout) = call_args_types.get(i).cloned() {
                        ...
                        let (layout, ref_type) = match tag_with_layout_info_opt.layout {
                            None => (None, ReferenceType::Value),
                            Some((layout, ref_type)) => (
                                Some(layout),
                                ref_type
        ...
        fn open_instruction_(
            ...
            match &frame.function.code()[pc as usize] {
                ...
                B::ReadRef => {
                    ...
                    let value =
                        self.resolve_location(&runtime_location, Some(frame), interpreter)?;
                    self.register_pre_effects(vec![
                        IF::Pop(ref_value),
                        IF::Read(Read {
                            location,
        ...
        fn close_instruction_(
            ...
            match instruction {
                ...
                B::PackGeneric(sidx) => {
                    ...
                    let value = self.resolve_stack_value(Some(frame), interpreter, 0)?;
                    let effects = self.register_post_effects(vec![IF::Push(value)]);
                    let TypeTag::Struct(s_type) = loader.type_to_type_tag(&struct_type).ok()?
                    else {
                ...
                B::VecUnpack(_, n) => {
                    ...
                    for i in (0..*n).rev() {
                        let value =
                            self.resolve_stack_value(Some(frame), interpreter, i as usize)?;
                        effects.push(IF::Push(value));
                    }
                    let effects = self.register_post_effects(effects);
                    self.trace
        ...
        pub(crate) fn close_instruction(
                ...
                .is_none()
            {
                ...
                let pc = self
                    ...
                    .expect("PC always set by this point by `open_instruction`");
                let instruction = &frame.function.code()[pc as usize];
                let effects = self.register_post_effects(vec![IF::ExecutionError(error_string)]);
                // TODO: type params here?
                self.trace
                    .instruction(instruction, vec![], effects, remaining_gas, pc);
            } else if let Some(err) = err {
                self.trace
                    .effect(IF::ExecutionError(format!("{:?}", err.major_status())));
            }
        }
    }
            ...
            let get_types_for_sig = |si: SignatureIndex| -> Option<Vec<TagWithLayoutInfoOpt>> {
                ...
                signatures
                    ...
                    .map(|tok| {
                        ...
                        let layout = type_layout.map(|layout| (layout, ref_type));
